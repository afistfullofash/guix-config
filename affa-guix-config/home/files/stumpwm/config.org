...
#+begin_src lisp
  ...
  (defun ml-email-data ()
    (flet ((notmuch-count
  	...
  	(let* ((tnatkinson-email-count (notmuch-count "tag:tnatkinson95 and tag:unread and not tag:promotions"))
    	       (natalieatkinson-email-count (notmuch-count "tag:natalieatkinson95 and tag:unread and not tag:promotions"))
  ...
  (defun ml-email-space-bar (email-count)
    (ml-space-bar (format nil "~a"
  		   (get-bar-zone-color-string
  		    email-count
  		    5 10 15))))
