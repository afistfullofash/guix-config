...
StumpWM kinda gives us incomplete color formatters for magnitude so make some extra ones which close off that changes
  #+begin_src lisp
    ...
    (defun set-bar-med-color ()
      (setf *bar-med-color* (format nil "^2^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg))))

    (defun set-bar-hi-color ()
      (setf *bar-hi-color* (format nil "^3*^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg))))

    (defun set-bar-crit-color ()
      (setf *bar-crit-color* (format nil "^1*^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg))))

    (defun bar-med-color-closer ()
      (format nil "^9^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg)))

    (defun bar-hi-color-closer ()
      (format nil "^9^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg)))

    (defun bar-crit-color-closer ()
      (format nil "^9^(:bg \"~A\")" (slot-value (with-current-theme) 'mode-line-bg)))
  ...
  (setf *screen-mode-line-format*      
        (list
         ...
         '(:eval (let ((tnatkinson-email-count (parse-integer (trimmed-shell-command "notmuch count tag:tnatkinson95 and tag:unread and not tag:promotions")))
  		     ...
  		     (work-email-count (parse-integer (trimmed-shell-command "notmuch count tag:work and tag:unread"))))
  		 (if (> (+ tnatkinson-email-count natalieatkinson-email-count work-email-count) 0)
  		     (medium-priority-message
  		      (format nil " ~a ~a ~a Emails "
  			      ...
  			      (get-bar-zone-color-string work-email-count 5 10 15)))
  		   "")))
         '(:eval )
         "%M"))
