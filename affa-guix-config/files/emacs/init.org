...
Turn off mouse interface early in startup to avoid momentary display
        ...
        #+end_src
*** Disable Arrow Keys
These have become a bad default habit when the C-a, C-e etc bindings are actually more ergonomic
#+begin_src emacs-lisp
  (use-package emacs
    :config
    (dolist (k '("<left>" "<right>" "<up>" "<down>"
                 "<kp-left>" "<kp-right>" "<kp-up>" "<kp-down>"))
      (global-set-key (kbd k) #'ignore)))
    #+end_src
*** Backup Directorys
...
#+begin_src emacs-lisp
  (setq afistfullofash/convert-scheme-modules-to-package
        (kmacro "C-r d e f i n e - m o d u l e <return> ) C-b <return> # : u s e - m o d u l e C-s u s e - m o d u l e <return> C-s ( <return> C-b C-SPC C-s ) <return> C-w C-r # : u s e - m o d <return> M-f M-f SPC C-y"))
#+end_src
** Spell Check
#+begin_src emacs-lisp
  (use-package jinx
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    :config
    (lsp-register-client
     (make-lsp-client :new-connection (lsp-stdio-connection '("tofu-ls" "serve"))
                      :major-modes '(terraform-mode)
                      :server-id 'tofu-ls
                      ;; This priority flag is the magic bullet. It forces lsp-mode 
                      ;; to choose tofu-ls over the built-in terraform-ls.
                      :priority 1))
    :magic (".svelte$" . lsp)
    :commands lsp)
...
#+begin_src emacs-lisp
  (use-package notmuch
    :custom
    (notmuch-saved-searches '((:name "Unread" :query "tag:unread and not tag:work")
  			    (:name "Unread with Work" :query "tag:unread")
  			    (:name "Work Important" :query "tag:work and tag:important and tag:unread")
...
*** Darkman
#+begin_src emacs-lisp
  (defun afistfullofash/disable-existing-themes (before no-theme-stacking activate)
    "Disable any activated themes before loading a new one."
    (mapc #'disable-theme custom-enabled-themes))

  (defun afistfullofash/darkman-switch-catppucin-properly (mode)
    "Configure theme-specific variables before darkman loads the theme."
    (cond
     ;; When switching to Light Mode
     ((eq mode :light)
      (message "Setting catppucin extra vars")
      (setq catppuccin-flavor 'latte)
      (catppuccin-reload))
     ;; When switching to Dark Mode (The Inverse)
     ((eq mode :dark)
      )))

  (use-package darkman
    :config
    (advice-add 'darkman-set :before #'afistfullofash/disable-existing-themes)
    (advice-add 'darkman-set :after #'afistfullofash/darkman-switch-catppucin-properly)
    :init
    (darkman-mode)
    :custom
    (darkman-themes '(:light catppuccin :dark doom-dracula)))
#+end_src
