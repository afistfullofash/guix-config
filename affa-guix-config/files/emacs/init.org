...
#+begin_src emacs-lisp
  (defun afistfullofash/convert-scheme-modules-to-package ()
    (interactive)
    (let ((macro-steps (kbd "C-r d e f i n e - m o d u l e <return> ) C-b <return> # : u s e - m o d u l e C-s u s e - m o d u l e <return> C-s ( <return> C-b C-SPC C-s ) <return> C-w C-r # : u s e - m o d <return> M-f M-f SPC C-y")))
      (execute-kbd-macro macro-steps)))
#+end_src
** Spell Check
#+begin_src emacs-lisp
  (use-package jinx
...
#+begin_src emacs-lisp
  (use-package notmuch
    ...
    (notmuch-saved-searches '((:name "Unread" :query "tag:unread")
  			    ...
  			    (:name "Unread Work Important" :query "tag:work and tag:important and tag:unread"))))
#+end_src
** Visual
*** Expand Region
#+begin_src emacs-lisp
...
*** Darkman
#+begin_src emacs-lisp
  (defun afistfullofash/darkman-pre-load-theme (theme &rest _)
    "Run right before Darkman loads THEME."
    (message "[darkman] loading theme %S" theme)
    (mapc #'disable-theme custom-enabled-themes)
    (when (eq theme 'catppuccin)
      ;; Catppuccin recommends setting flavor before loading the theme.
      (setq catppuccin-flavor 'latte)))

  (use-package darkman
    :init
    (advice-add #'darkman--load-theme :before #'afistfullofash/darkman-pre-load-theme)
    (darkman-mode)
    :custom
