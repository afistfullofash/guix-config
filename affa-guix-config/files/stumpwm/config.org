...
Start slynk for debugging purposes first and then load in our required stumpwm contrib modules
  #+begin_src lisp
    ...
    ;; CLX extensions (RandR, Render, etc.) export symbols beyond what CLX's
    ;; DEFPACKAGE declares, causing SBCL to signal a package-variance error
    ;; when ASDF recompiles CLX source files. Downgrade to warning only.
