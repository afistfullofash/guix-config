...
Start slynk for debugging purposes first and then load in our required stumpwm contrib modules
  #+begin_src lisp
    ...
    (if *initializing*
        (progn
          ;; CLX is already compiled into the StumpWM image. When ASDF
          ;; resolves dependencies for contrib modules (pamixer, etc.),
          ;; it traverses stumpwm -> clx and tries to recompile CLX from
          ;; source, which fails due to DEFPACKAGE variance with the
          ;; already-loaded XLIB package. Mark these systems as immutable
          ;; so ASDF skips them entirely.
          (asdf:register-immutable-system :clx)
